...
class TestCategoryListView:
    ...
    def test_category_list_view_first_page(self, get_data, response_keys):
        ...
        assert response.data['results'] == GoalCategorySerializer(categories[:self.PAGE_SIZE], many=True).data, \
    ...
    def test_category_list_view_middle_page(self, get_data, response_keys):
        ...
        assert response.data['count'] == self.COUNT, 'Неверное количество записей'
        assert len(response.data['results']) == self.PAGE_SIZE, 'Вернулось не то количество элементов'
        assert response.data['results'] == GoalCategorySerializer(categories[self.PAGE_SIZE:self.PAGE_SIZE * 2],
    ...
    def test_category_list_view_last_page(self, get_data, response_keys):
        ...
        assert response.data['results'] == GoalCategorySerializer(categories[-(self.COUNT % self.PAGE_SIZE):],
                                                              many=True).data, f'Вернулись неверные данные'
    ...
    def test_category_list_view_search(self, get_data, response_keys):
        ...
        assert response.data['results'] == GoalCategorySerializer([category], many=True).data, f'Вернулись неверные данные'
